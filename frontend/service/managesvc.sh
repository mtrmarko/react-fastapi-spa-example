...
TEMPLATE_PATH=$WORKING_DIR/$SVC_TEMPLATE_NAME
TEMP_PATH=$WORKING_DIR/$SVC_NAME.temp
CONFIG_PATH=.service
...
function install()
{
    ...
    chown ${run_as_uid}:${run_as_gid} ./startsvc.sh || failed "failed to set owner for startsvc.sh"
    chmod 755 ./startsvc.sh || failed "failed to set permission for startsvc.sh"
